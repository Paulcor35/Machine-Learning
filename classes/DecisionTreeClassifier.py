...
class _Node:
    is_leaf: bool
    prediction: object | None = None       # predicted class if node is a leaf
    attr: int | None = None                # index of the feature used for split
    threshold: float | None = None         # threshold for numeric splits
    branches: dict | None = None           # children nodes: {"le": _Node, "gt": _Node} for numeric or {val: _Node} for categorical


def _entropy(y: np.ndarray) -> float:
    """
    Computes the entropy of a target vector y.

    Entropy is used as the splitting criterion in classification (information gain).
    """
    if y.size == 0:
        return 0.0
    vals, cnt = np.unique(y, return_counts=True)
    p = cnt.astype(np.float64) / y.size
    # only compute log2 for non-zero probabilities
...
def _best_split_vectorized(
    ...
    """
    Finds the best split for a classification node based on entropy (information gain).

    Numeric columns:
        - candidate thresholds are midpoints between consecutive distinct values,
          optionally subsampled by `max_thresholds`.
    Categorical columns:
        - gain is computed for partition by category value.

    Returns:
        (best_feature_index, best_threshold, best_gain)
        If split is categorical, threshold=None.
    """
    n, p = X.shape
    base_h = _entropy(y)                  # entropy at current node
    best_gain, best_attr, best_thr = -1.0, None, None

    for j in range(p):
        col = X[:, j]

        # Numeric feature with more than 2 unique values
        if np.issubdtype(col.dtype, np.number) and np.unique(col).size > 2:
            ...
            # candidate split positions: where value changes
            diff = np.diff(xs)
            ...
            gains = base_h - new_H                         # information gain
            ...
            if vals.size <= 1:
                continue
            # weighted entropy for each branch
            new_H = 0.0
            for v, c in zip(vals, counts):
                if c < min_samples_leaf or (n - c) < min_samples_leaf:
                    # branch too small: ignore split
                    new_H = base_h
                    break
                y_sub = y[col == v]
...
class DecisionTreeClassifier:
    """
    Decision tree classifier using entropy (information gain) and vectorized splits.

    - Recursively builds a binary or multi-branch tree for categorical features.
    - Leaves predict the majority class among samples in the leaf.
    - Vectorized operations for efficient training with prefix sums.
    - Regularization safeguards: min_samples_leaf, min_gain, max_depth, min_samples_split.
    """
    typ = 'c'
    ...
    def _plurality(self, y: np.ndarray):
        """Return the most common class in y (majority vote)."""
    ...
    def _build(self, X: np.ndarray, y: np.ndarray, depth: int) -> _Node:
        ...
        - Stops when node is pure, max_depth reached, or insufficient samples.
        - Chooses the best split using `_best_split_vectorized`.
        """
        n = y.size
        if n == 0:
        ...
        if thr is None:
            # categorical split: one branch per category
            branches = {}
            ...
            # numeric split: "le" = left, "gt" = right
            mask_le = X[:, attr] <= thr
    ...
    def _majority_subtree(self, node: _Node):
        """
        Used for handling unseen category values at prediction time.
        Returns a leaf predicting the majority class among all descendants.
        """
        counts = Counter()
        stack = [node]
        while stack:
    ...
    def _predict_one(self, node: _Node, x: np.ndarray):
        """Traverse the tree to predict a single sample x."""
    ...
    def predict(self, X):
        ...
        if self._tree is None:
            raise RuntimeError("DecisionTreeClassifier not trained, call fit first.")
        X = np.asarray(X)
